...
class User(db.Model):
    ...
    email = db.Column(db.String(100), unique=True, nullable=False)  # Typo was here
...
# Order model and routes remain the same
...
def create_order():
    data = request.get_json()
    required_fields = ['description', 'contact', 'pickup_location', 'delivery_location', 'delivery_time', 'delivery_date']
    
    # Check if all required fields are present in the request data
    if not all(field in data for field in required_fields):
        return jsonify({'message': 'Missing required fields'}), 400

    # Ensure pickup_location is present in the request data
    ...
    order = Order(
        ...
        pickup_location=data['pickup_location'],  # Use 'pickup_location' instead of 'pickupLocation'
